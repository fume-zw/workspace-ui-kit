...
- 下部: 選択日の**統合アジェンダ** — 期限タスク（定期タスク各回を含む）・イベント・生活・勤務予定・定期スケジュール・記録を**時刻順**に一覧。各行は時刻 + タイトル + 種類バッジ（タスク / イベント / 生活 / 勤務 / 定期 / 記録）。
...
| `20260821000011_activity_labels.sql` | 定期スケジュールを `activity_labels` + `kind=activity` に分離。既存の定期ラベルを移す |
| `20260821000012_record_labels.sql` | 記録ラベル（睡眠・出勤・帰宅）+ `kind=record`。生活から睡眠を外す。記録は ICS に出さない |
