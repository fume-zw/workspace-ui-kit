...
**最終更新:** 2026-06-11（レポート本番確認完了・web/ 削除・Actions Variables 整理手順追記）
...
1. **GitHub Actions Variables の削除** — `gh auth login` 後、`INFORMATION.md` H 節の `gh variable delete` コマンドで `STATUS_*` / `COMPLETION_PENDING_PATTERN` を削除
...
**workspace-ui-kit** — Auth / mobile / Supabase CRUD / migrations / handoff など大量の変更がローカルのみ。

**自動報告ツール** — レポート改修・`web/` 削除・`preview-report.mjs`・`test/` など。`.env` はローカルのみ（コミットしない）。

### 再開時の最初の一手（推奨順）

1. **GitHub Actions Variables 削除**（`gh auth login` → `INFORMATION.md` H 節の 4 コマンド）
2. **両リポジトリを commit & push**（秘密情報 `.env` / `.env.local` は含めない）
