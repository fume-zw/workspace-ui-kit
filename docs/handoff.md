...
2026-08-20 に利用イメージ（Watch = 予定確認+音声タスク追加、iPhone/Windows = 確認・編集）を反映して書き直した。実装は承認後。

- **正本:** [spec-apple-devices.md](./spec-apple-devices.md)
- **相談の結論:** タスク管理の正本は自作。Google / Apple カレンダーは同じ ICS を購読する「見え方」だけ
- **Watch:** 純正カレンダーで予定確認。音声追加は Siri ショートカット → inbox API（専用 Watch アプリは v1 では作らない）
- **やらないこと（v1）:** カレンダーからアプリへの書き戻し、手首での完了、音声での予定追加
