...
2026-08-20 確定。実装は inbox から。

- **正本:** [spec-apple-devices.md](./spec-apple-devices.md)
- **音声:** Siri ショートカット。プロジェクトは分けず **すべて未割当**。期限は言ったときだけ
- **あとから直す:** iPhone `/mobile` または Windows ブラウザでプロジェクト割当・編集
- **カレンダー:** まだ使わない。使うときは同じ ICS を購読（おすすめどおり）
