...
| `20260821000011_activity_labels.sql` | 定期を `activity_labels` + `kind=activity` に分離。既存の `category='activity'` を移す |
| `20260821000012_record_labels.sql` | 記録ラベル（睡眠・出勤・帰宅）+ `kind=record`。生活から睡眠を外す |
...
| **「＋新規作成」** | 検索バー横 → ボタン1つ+メニュー。タスク画面=「タスク/定期タスク」、スケジュール画面=「イベント/生活/勤務予定/定期スケジュール/記録」 |
...
| `20260821000011_activity_labels.sql` | 定期スケジュールを勤務から分離（`activity_labels` + `kind=activity`）。既存の定期ラベルを移す |
| `20260821000012_record_labels.sql` | 記録枠（睡眠・出勤・帰宅）。生活から睡眠を外す。記録はタイトルなし・ICS 非掲載 |
...
| `20260821000011_activity_labels.sql` | `activity_labels` + `kind=activity`。勤務と定期は別テーブル |
| `20260821000012_record_labels.sql` | `record_labels` + `kind=record`（睡眠・出勤・帰宅）。生活はお風呂・食事のみ |
...
- **音声:** Siri ショートカット「追加」。Watch 正は `GET /api/inbox?token=…&text=…`（POST + JSON + Authorization は Siri/Watch で通信切断になりやすい）。「スケジュールに入れて」→ 予定、「タスクに入れて」→ 未割当タスク。言わなければ開始時刻で推定。**「おやすみ」「おはよう」** は記録枠の睡眠。**「いってきます」「ただいま」** は出勤・帰宅の記録バー。**「お風呂」「食事」** は生活。言わなかった日は作らない（ヘルスケア連携はしない）
- **あとから直す:** iPhone `/mobile` または Windows でプロジェクト割当・時刻の微修正
- **カレンダー:** 同じ ICS を購読する。`SUMMARY` はタイトルだけ（ラベル名や「タスク」接頭辞は付けない。Apple カレンダーで長いタイトルが切れるのを避ける）。**記録（睡眠・出勤・帰宅）は ICS に出さない**
