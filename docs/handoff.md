...
- **音声:** Siri ショートカット「追加」。Watch 正は `GET /api/inbox?token=…&text=…`（POST + JSON + Authorization は Siri/Watch で通信切断になりやすい）。「スケジュールに入れて」→ 予定、「タスクに入れて」→ 未割当タスク。言わなければ開始時刻で推定。**「おやすみ」「おはよう」** は記録枠の睡眠。**「お風呂」「食事」** は生活。言わなかった日は作らない（ヘルスケア連携はしない）
